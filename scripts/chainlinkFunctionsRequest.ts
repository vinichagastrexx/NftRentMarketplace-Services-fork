...
async function main() {
  ...
  const signerPrivateKey = env.walletPvKey;
  const signer = new ethers.Wallet(signerPrivateKey, provider);

  const consumerAddress = '0xbFfb8EF88bcb35Ac25F6ab090907046559739E19';
  const consumerAbiPath =
    './artifacts/contracts/NFTRentMarketplace.sol/NFTRentMarketplace.json';
